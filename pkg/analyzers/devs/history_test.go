...
func TestDevsHistoryAnalyzer_Consume(t *testing.T) {
	...
	d.Consume(&analyze.Context{Commit: commit1})
	...
	d.Consume(&analyze.Context{Commit: &object.Commit{Hash: gitplumbing.NewHash("c2")}})
	if dev.Commits != 1 {
	...
	d.Consume(&analyze.Context{Commit: &object.Commit{Hash: gitplumbing.NewHash("c3")}})
	if dev.Commits != 2 {
	...
	d.Consume(&analyze.Context{Commit: commitMerge})
	...
	d.Consume(&analyze.Context{Commit: commitMerge, IsMerge: true})
	if dev.Commits != 3 {
...
func TestDevsHistoryAnalyzer_Finalize(t *testing.T) {
	...
	ticks := report["Ticks"].(map[int]map[int]*DevTick)
