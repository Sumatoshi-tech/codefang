...
func TestBurndownHistoryAnalyzer_Consume_Insert_Exists(t *testing.T) {
	...
	shard.files[name], _ = b.newFile(shard, hash1, name, 0, 0, 1)
...
func TestBurndownHistoryAnalyzer_Modify_Binary(t *testing.T) {
	...
	shard1.files[name1], _ = b.newFile(shard1, hashText, name1, 0, 0, 1)
...
func TestBurndownHistoryAnalyzer_Consume_Hibernated(t *testing.T) {
	...
	name := "test.txt"
	shard := b.getShard(name)
	shard.files[name], _ = b.newFile(shard, gitplumbing.ZeroHash, name, 0, 0, 1)
...
func TestBurndownHistoryAnalyzer_Consume_Merge_Paths(t *testing.T) {
	...
	b.Consume(&analyze.Context{IsMerge: true})
	...
	name2 := "modify.txt"
	shard2 := b.getShard(name2)
	shard2.files[name2], _ = b.newFile(shard2, hash1, name2, 0, 0, 1)
	...
	b.FileDiff.FileDiffs = map[string]pkgplumbing.FileDiffData{
		name2: {OldLinesOfCode: 1, NewLinesOfCode: 2, Diffs: []diffmatchpatch.Diff{{Type: diffmatchpatch.DiffEqual, Text: "A"}, {Type: diffmatchpatch.DiffInsert, Text: "B"}}},
	}
	...
	shard3.files[name3], _ = b.newFile(shard3, hash1, name3, 0, 0, 1)
	...
	shard4old.files[name4old], _ = b.newFile(shard4old, hash1, name4old, 0, 0, 1)
