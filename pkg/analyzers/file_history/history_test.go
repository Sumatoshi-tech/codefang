...
func TestFileHistoryAnalyzer_Consume(t *testing.T) {
	...
	h.Consume(&analyze.Context{Commit: commit1})
	...
	h.Consume(&analyze.Context{Commit: commit2})
	...
	h.Consume(&analyze.Context{Commit: commit3})
	...
	h.Consume(&analyze.Context{Commit: commit4})
...
func TestFileHistoryAnalyzer_Misc(t *testing.T) {
	...
	c1 := clones[0].(*FileHistoryAnalyzer)
	if len(c1.files) != 1 {
