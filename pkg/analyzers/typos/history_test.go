...
func TestTyposHistoryAnalyzer_Consume(t *testing.T) {
	...
	// UAST
	// Need Identifier nodes on line 2 (0-indexed lines in blob split? code uses line-1 for array access)
	// In Consume:
	// linesBefore := bytes.Split(blobBefore.Data, []byte{'\n'})
	// ... line := int(id.Pos.StartLine) - 1
	// ... nodesBefore := removedIdentifiers[c.Before]
	// c.Before is index in linesBefore.
	...
	// case diffmatchpatch.DiffInsert: if size == removedSize ... checks distance
