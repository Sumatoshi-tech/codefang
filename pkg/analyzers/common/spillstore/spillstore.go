...
// SliceSpillStore wraps a []V with transparent disk spilling.
// Used for analyzers that accumulate ordered lists (e.g. typos).
type SliceSpillStore[V any] struct {
	current []V
	dir     string
	spillN  int
}

// NewSlice creates a SliceSpillStore with an empty buffer.
func NewSlice[V any]() *SliceSpillStore[V] {
	return &SliceSpillStore[V]{}
}

// Append adds values to the current in-memory buffer.
func (s *SliceSpillStore[V]) Append(vals ...V) {
	s.current = append(s.current, vals...)
}

// Len returns the number of entries in the current in-memory buffer.
// Safe to call on a nil receiver (returns 0).
func (s *SliceSpillStore[V]) Len() int {
	if s == nil {
		return 0
	}
...
// Spill writes the current buffer to a numbered gob file and clears the slice.
// No-op if the buffer is empty or the store is nil.
func (s *SliceSpillStore[V]) Spill() error {
	if s == nil || len(s.current) == 0 {
		return nil
	}
...
// Collect returns all data (spilled + in-memory) concatenated in order.
// After this call, spill files are cleaned up.
func (s *SliceSpillStore[V]) Collect() ([]V, error) {
	var result []V
...
// SpillCount returns the number of spill files written.
// Safe to call on a nil receiver (returns 0).
func (s *SliceSpillStore[V]) SpillCount() int {
	if s == nil {
		return 0
	}
...
// Cleanup removes the temp directory. Safe to call multiple times.
func (s *SliceSpillStore[V]) Cleanup() {
	if s.dir != "" {
		os.RemoveAll(s.dir)
		s.dir = ""
	}
...
func (s *SliceSpillStore[V]) readSpillFile(index int) ([]V, error) {
	path := filepath.Join(s.dir, fmt.Sprintf("chunk_%03d.gob", index))
