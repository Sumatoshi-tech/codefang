...
func TestFormatter_formatCollectionTable_Empty(t *testing.T) {
	formatter := NewFormatter(FormatConfig{})
	result := formatter.formatCollectionTable("items", []map[string]interface{}{})
	if result != "" {
...
func TestFormatter_getCollectionKeys_Empty(t *testing.T) {
	formatter := NewFormatter(FormatConfig{})
	keys := formatter.getCollectionKeys([]map[string]interface{}{})
	if len(keys) != 0 {
...
func TestFormatter_toComparable(t *testing.T) {
	...
	tests := []struct {
		...
		{"string", "hello", 5.0}, // Length of string
		{"nil", nil, 0},
	}
