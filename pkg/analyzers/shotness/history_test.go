...
func TestShotnessHistoryAnalyzer_FinalizeAndSerialize(t *testing.T) {
	...
	if len(report["Nodes"].([]NodeSummary)) != 2 {
...
func TestShotnessHistoryAnalyzer_Consume_Rename(t *testing.T) {
	...
	// Seed state
	funcNode := &node.Node{ID: "1", Type: "Function", Token: "Func", Roles: []node.Role{node.RoleFunction}, Pos: &node.Positions{StartLine: 1, EndLine: 1}}
	...
	// Rename
	funcNode2 := &node.Node{ID: "2", Type: "Function", Token: "Func", Roles: []node.Role{node.RoleFunction}, Pos: &node.Positions{StartLine: 1, EndLine: 1}}
