...
func TestTreeDiffAnalyzer_Consume_Full(t *testing.T) {
	...
	commit2.Encode(cObj2)
	commitHash2, _ := r.Storer.SetEncodedObject(cObj2)
	realCommit2, _ := r.CommitObject(commitHash2)
...
func TestFileDiffAnalyzer_Consume_Full(t *testing.T) {
	...
	fd.Initialize(nil)
	fd.Configure(map[string]interface{}{
...
func TestIdentityDetector_Consume(t *testing.T) {
	...
	id.Consume(&analyze.Context{Commit: commit1})
	if id.AuthorID != 1 {
	...
	id.Consume(&analyze.Context{Commit: commit2})
...
func TestTicksSinceStart_Consume(t *testing.T) {
	...
	ts.Consume(&analyze.Context{Commit: commit1, Index: 0})
	...
	ts.Consume(&analyze.Context{Commit: commit2, Index: 1})
...
func TestBlobCacheAnalyzer_Consume_Full(t *testing.T) {
	...
	w.Write(content)
	w.Close()
	blobHash, _ := r.Storer.SetEncodedObject(obj)
	...
	commit.Encode(cObj)
	commitHash, _ := r.Storer.SetEncodedObject(cObj)
	
	realCommit, _ := r.CommitObject(commitHash)
