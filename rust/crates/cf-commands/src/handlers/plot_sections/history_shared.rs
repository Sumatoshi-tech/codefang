//! Shared helpers for the history-analyzer plot sections: the Go display
...
/// Go `fmt.Sprintf("%.<prec>f", v)` (round-half-to-even is NOT what Go does —
/// `fmt` rounds half away from zero via strconv shortest-then-fixed; Rust's
/// `format!("{:.prec$}")` matches Go's fixed-precision decimal rounding for
...
/// Go devs `formatNumber`: `1.5K` / `2.3M` shorthand above the thousand /
...
pub fn format_number(n: i64) -> String {
    ...
    if n >= MILLION {
        // Go: strconv.FormatFloat(n/1e6, 'f', 1, 64) + "M".
...
/// Go devs `formatSignedNumber`: positive values carry a `+` prefix.
#[must_use]
...
/// Go burndown `formatInt64`: thousands separated by commas (`2,703`).
#[must_use]
...
impl GridStats {
    ...
    /// Appends a plain stat (Go `plotpage.NewStat`).
    #[must_use]
    ...
    /// Appends a stat with a trend badge (Go `NewStat(..).WithTrend(..)`).
    #[must_use]
    ...
    /// Finishes the grid (Go `plotpage.NewGrid`).
    #[must_use]
