//! The `codefang version` subcommand — Rust port of Go `versionCmd`
//! (`cmd/codefang/main.go:301`).
//!
//! Go prints, on STDOUT, with exit code 0 and no flags:
//!
//! ```text
...
//! reproduces the Go defaults `dev` / `none` / `unknown` when nothing is
//! injected at build time. This subcommand is a Layer-D CLI golden target
//! (DESIGN §6), so the output must be byte-identical to Go.
...
/// built: <date>)\n`. Mirrors Go `fmt.Fprintf(os.Stdout, "codefang %s (commit:
/// %s, built: %s)\n", ...)`.
#[must_use]
...
///
/// Go's `versionCmd` is `Use: "version"`, `Short: "Show codefang version"`, with
...
mod tests {
    ...
    fn version_output_matches_go_format_with_defaults() {
        // With no build-time injection cf-version uses Go's dev/none/unknown.
        assert_eq!(
            version_output(),
