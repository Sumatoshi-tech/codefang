...
//!
//! Port of Go `internal/streaming/hibernatable.go`.
...
///
/// Port of Go `streaming.SpillCleaner`.
...
///
/// Port of Go `streaming.SpillCleanupGuard`, which embeds
/// `sigutil.SignalCleanupGuard`. This wraps [`cf_sigutil::SignalCleanupGuard`]
/// the same way: that type owns the run-exactly-once semantics
/// (`sync.Once`-equivalent), the [`Drop`] hook, and SIGTERM/SIGINT registration.
...
impl SpillCleanupGuard {
    ...
    ///
    /// Port of Go `NewSpillCleanupGuard`. Cleaners must be `Send` because the
    /// underlying guard may run cleanup from a signal-handling thread.
    #[must_use]
    ...
    /// Creates a guard from boxed trait objects, allowing a heterogeneous set of
    /// cleaners. Equivalent to the Go `[]SpillCleaner` slice form.
    #[must_use]
    ...
    ///
    /// Port of Go `(*SignalCleanupGuard).Close`.
...
/// streaming execution.
///
/// Port of Go `streaming.Hibernatable`.
...
///
/// Go uses the bare `error` interface; this crate provides a concrete,
/// inspectable error so analyzers across crates can interoperate. It carries a
...
mod tests {
    ...
    // Mirrors Go TestSpillCleanupGuard_CleanupOnClose.
    #[test]
    fn cleanup_on_close() {
    ...
    // Mirrors Go TestSpillCleanupGuard_IdempotentClose.
    #[test]
    fn idempotent_close() {
    ...
    // Mirrors Go TestSpillCleanupGuard_NilCleaners (empty cleaner set).
    #[test]
    fn nil_cleaners() {
    ...
    // Mock hibernatable mirroring the Go hibernatable_test.go mock.
    ...
    // Mirrors Go TestHibernateAnalyzers_SingleChunk_NoHibernation.
    #[test]
    ...
    // Mirrors Go TestHibernateAnalyzers_MultipleChunks_Hibernates.
    #[test]
    ...
    // Mirrors Go TestCollectHibernatables_MixedAnalyzers.
    #[test]
