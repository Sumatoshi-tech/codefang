...
//!
//! The seeds and hash values produced here are a frozen compatibility
//! contract: sketch estimates derived from them appear in machine-format
//! reports whose bytes are pinned against the reference implementation by
//! `rust/tests/compat`. Do not change the constants, seed schedule, or mixing
//! algebra.
//!
//! # Wrapping arithmetic
//!
//! All multiplications and additions use explicit [`u64::wrapping_mul`] /
//! [`u64::wrapping_add`] so results are identical in **every** build profile
//! (overflow would panic in debug builds otherwise). XOR and shifts never
//! overflow.
//!
//! # Examples
//!
...
/// function.
///
/// Kept `pub(crate)` so the constant is documented and testable without
...
#[inline]
#[must_use]
pub const fn mix64(mut v: u64) -> u64 {
    v ^= v >> MIX_SHIFT1;
...
#[inline]
#[must_use]
pub const fn mix_hash(base: u64, seed: u64) -> u64 {
    let mut x = base ^ seed;
...
///
/// Canonical FNV-1a/64: start from the 64-bit offset basis, then for each byte
/// XOR it into the low byte of the accumulator and multiply by the 64-bit FNV
/// prime (wrapping).
#[inline]
#[must_use]
...
///
/// Use [`mix64`] for CMS-style seeds or [`splitmix64`] for MinHash-style
/// seeds. The function threads `BASE_SEED` through `advance`, collecting each
/// advanced state as a seed; this ordering is part of the frozen seed
/// schedule. `advance` is any `Fn(u64) -> u64`, so both bare function items
/// (`mix64`, `splitmix64`) and closures are accepted.
#[must_use]
...
mod tests {
    ...
    // 0 is a fixed point of the multiplicative finalizer.
    #[test]
    ...
    // No short cycle over 100 iterations starting from BASE_SEED.
    #[test]
    ...
    // XOR is commutative, so mix_hash is symmetric in its arguments.
    #[test]
    ...
    // Empty input hashes to the offset basis, which is non-zero.
    #[test]
    ...
    // Parity guard: pin canonical FNV-1a 64-bit known-answer values. These
    ...
    // generate_seeds also accepts closures (the bare-fn path is the
    // parity-critical one).
    #[test]
    ...
    // Verify the well-known splitmix64 constants (frozen contract).
    #[test]
    ...
    // Pin known-answer values for the mixers so any accidental algebra change
    // is caught locally. These are the exact reference-implementation outputs
    // for the given inputs, anchoring bit-identity (DESIGN.md §2.6).
    #[test]
    fn mixer_known_answers() {
        ...
        // mix_hash(0xDEADBEEF, 0) cross-checked against the reference oracle.
        assert_eq!(mix_hash(0xDEAD_BEEF, 0), 0x4e06_2702_ec92_9eea);
    }

    // Pin the first three generate_seeds outputs for both advance functions
    // against the reference oracle, anchoring byte-identity of the seed
    // sequences that feed CMS/HLL/MinHash.
    #[test]
