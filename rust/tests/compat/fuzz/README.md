...
(`rust/target/release/{codefang,uast}`) under the pinned env, and **FAILS on any
...
rust/tests/compat/fuzz/run_fuzz.sh 20

# one target, longer:
FUZZ_ONLY=FuzzParseGo rust/tests/compat/fuzz/run_fuzz.sh 60

# raw go test (seed pass only, no mutation):
go test ./rust/tests/compat/fuzz/ -run FuzzParseGo -count=1
# active mutation fuzz of one target:
go test ./rust/tests/compat/fuzz/ -run '^$' -fuzz '^FuzzParseGo$' -fuzztime 30s
```
