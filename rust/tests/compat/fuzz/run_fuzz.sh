...
#   rust/tests/compat/corpus/fuzzfinds/
...
echo "budget=${BUDGET}s/target  oracle=build/bin  candidate=rust/target/release"
echo
...
echo
echo "================ RESULT ================"
FINDS=$(ls "$ROOT/rust/tests/compat/corpus/fuzzfinds/" 2>/dev/null | grep -vcE '\.(evidence\.json|rust_out)$|\.go_run_' || echo 0)
echo "distilled divergence inputs: ${FINDS} (rust/tests/compat/corpus/fuzzfinds/)"
