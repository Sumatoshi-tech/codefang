...
//
// Pre-builds a single global Bloom filter of shared variables (variables appearing
// in more than one function), then tests each function's variables against it.
// This reduces O(N² × M) work to O(N × M) with O(1) Bloom lookups.
func (c *Analyzer) computePerFunctionCohesion(functions []Function) {
	globalFilter := buildGlobalVariableFilter(functions)

	for i := range functions {
		functions[i].Cohesion = c.calculateFunctionLevelCohesion(functions[i], globalFilter)
	}
}
