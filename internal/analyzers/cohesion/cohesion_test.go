...
func TestVariableSharingRatio(t *testing.T) {
	...
	globalFilter := buildGlobalVariableFilter(allFunctions)

	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()

			result := analyzer.calculateFunctionLevelCohesion(tc.fn, globalFilter)
			assert.InDelta(t, tc.expected, result, 0.001,
...
func TestAnalyzer_EdgeCases(t *testing.T) {
	...
	gf := buildGlobalVariableFilter(allFns)
	cohesion := analyzer.calculateFunctionLevelCohesion(allFns[0], gf)
