...
func (b *HistoryAnalyzer) Hibernate() error {
	...
	for i, shard := range b.shards {
		...
		// Shrink treap node pools before spilling to release excess free-list
		// memory to the GC. Each file's pool accumulates released nodes during
		// Replace operations; trimming them here avoids retaining stale pointers.
	...
	// Compact rename tracking maps by copying only live entries.
	// Stale entries (from → "") accumulate from deletions but are never
	// cleaned up, wasting memory proportional to total renames in history.
	b.GlobalMu.Lock()
	b.compactRenameMaps()
	b.GlobalMu.Unlock()
