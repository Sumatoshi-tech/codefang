...
func (allocator *Allocator) Clone() *Allocator {
	...
	newAllocator := &Allocator{
		...
		gaps:                 slices.Clone(allocator.gaps),
		hibernatedData:       [7][]byte{},
		hibernatedStorageLen: 0,
		hibernatedGapsLen:    0,
	}
...
func (allocator *Allocator) Hibernate() {
	...
	// Delta-encode Key array for better LZ4 compression of sorted keys.
	DeltaEncodeUInt32Slice(buffers[0])
	...
	go func() {
		if len(allocator.gaps) > 0 {
			...
			allocator.hibernatedData[len(buffers)] = CompressUInt32Slice(allocator.gaps)
		}
...
func (allocator *Allocator) Boot() {
	...
	go func() {
		if allocator.hibernatedGapsLen > 0 {
			...
			allocator.gaps = make([]uint32, allocator.hibernatedGapsLen)
			DecompressUInt32Slice(gapData, allocator.gaps)
	...
	// Delta-decode Key array to restore original values.
	DeltaDecodeUInt32Slice(buffers[0])

	allocator.reinterleave(buffers)
}

// reinterleave rebuilds the storage slice from decompressed field buffers.
func (allocator *Allocator) reinterleave(buffers [6][]uint32) {
...
func (allocator *Allocator) malloc() uint32 {
	...
	if len(allocator.gaps) > 0 {
		lastIdx := len(allocator.gaps) - 1
		key := allocator.gaps[lastIdx]
		allocator.gaps = allocator.gaps[:lastIdx]
...
func (allocator *Allocator) free(nodeIdx uint32) {
	...
	allocator.gaps = append(allocator.gaps, nodeIdx)
}
