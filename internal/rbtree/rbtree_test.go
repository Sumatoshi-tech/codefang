...
func TestAllocatorHibernateBoot(t *testing.T) {
	...
	for idx := range 10000 {
		alloc.gaps = append(alloc.gaps, uint32(idx)) // Makes no sense, only to test.
	}
	...
	for nd := 1; nd <= 10000; nd++ {
		...
		assert.Contains(t, alloc.gaps, uint32(nd-1))
	}
}
...
// Free-list stack test constants.
const (
	stressTestCycles = 100000
	benchMallocFree  = 100000
)

// TestFreeListStack_Stress verifies no leaks across many insert/delete cycles.
func TestFreeListStack_Stress(t *testing.T) {
	t.Parallel()
	...
	for i := range stressTestCycles {
		tree.Insert(Item{uint32(i), uint32(i)})
	}

	assert.Equal(t, stressTestCycles, tree.Len())

	// Delete all elements.
	for i := range stressTestCycles {
		tree.DeleteWithKey(uint32(i))
	}

	assert.Equal(t, 0, tree.Len())

	// All freed indices should be in gaps.
	assert.Len(t, alloc.gaps, stressTestCycles)

	// Re-insert: all should reuse freed indices.
	for i := range stressTestCycles {
		tree.Insert(Item{uint32(i + stressTestCycles), uint32(i)})
	}

	assert.Equal(t, stressTestCycles, tree.Len())
	assert.Empty(t, alloc.gaps)

	// Random insert/delete cycles.
	for range stressTestCycles {
		op := rng.int32n(2)

		switch op {
		case 0:
			key := uint32(rng.int32n(int32(stressTestCycles)))
	...
	// Verify consistency.
	expectedUsed := alloc.Size() - len(alloc.gaps)
	assert.Equal(t, expectedUsed, alloc.Used())
}

// BenchmarkMallocFree_Stack benchmarks malloc/free with slice-based free list.
func BenchmarkMallocFree_Stack(b *testing.B) {
	for range b.N {
		alloc := NewAllocator()

		indices := make([]uint32, 0, benchMallocFree)

		for range benchMallocFree {
			indices = append(indices, alloc.malloc())
		}
