...
// defaultPRNGSeed is the initial non-zero seed for the xorshift64 PRNG.
// Any non-zero value produces a full-period sequence (2^64 - 1).
const defaultPRNGSeed = uint64(0x2545_F491_4F6C_DD1D)

// xorshift64ShiftA is the first left-shift constant in the xorshift64 algorithm.
const xorshift64ShiftA = 13

// xorshift64ShiftB is the right-shift constant in the xorshift64 algorithm.
const xorshift64ShiftB = 7

// xorshift64ShiftC is the second left-shift constant in the xorshift64 algorithm.
const xorshift64ShiftC = 17

// xorshift64UpperShift extracts the upper 32 bits from a 64-bit state.
const xorshift64UpperShift = 32

// xorshift64 advances the PRNG state and returns a uint32 priority from the upper bits.
// The algorithm is Marsaglia's xorshift64 with period 2^64 - 1. State must be non-zero.
func xorshift64(state *uint64) uint32 {
	s := *state
	s ^= s << xorshift64ShiftA
	s ^= s >> xorshift64ShiftB
	s ^= s << xorshift64ShiftC
	*state = s

	return uint32(s >> xorshift64UpperShift)
}
...
// nodePool manages a free-list of treapNode pointers for reuse.
// Reduces heap allocation frequency and GC pressure by recycling nodes.
type nodePool struct {
	free []*treapNode
}

// acquire returns a zeroed node from the free-list, or allocates a new one.
func (p *nodePool) acquire() *treapNode {
	if n := len(p.free); n > 0 {
		node := p.free[n-1]
		p.free[n-1] = nil
		p.free = p.free[:n-1]
...
// release zeros all fields and returns the node to the free-list.
func (p *nodePool) release(node *treapNode) {
	if node == nil {
		return
	}
...
// shrink trims the free-list down to keep items.
// Excess nodes are nilled out so the GC can reclaim the treapNode structs.
func (p *nodePool) shrink(keep int) {
	if len(p.free) <= keep {
		return
	}
...
// releaseSubtree recursively releases all nodes in the subtree (post-order).
func (p *nodePool) releaseSubtree(node *treapNode) {
	if node == nil {
		return
	}

	p.releaseSubtree(node.left)
	p.releaseSubtree(node.right)
	p.release(node)
}
...
func NewTreapTimeline(time, length int) *treapTimeline {
	...
	t := &treapTimeline{totalLength: length, prngState: defaultPRNGSeed}
	if length > 0 {
...
// ShrinkPool trims the internal node pool to retain at most keep free nodes.
// Call between chunks to release excess pool memory to the GC.
func (tt *treapTimeline) ShrinkPool(keep int) {
	tt.pool.shrink(keep)
}
...
func (tt *treapTimeline) newNode(length int, value TimeKey) *treapNode {
	...
	n.value = value
	n.size = length
	n.priority = xorshift64(&tt.prngState)
...
func (tt *treapTimeline) splitByLines(root *treapNode, pos int) (left, right *treapNode) {
	...
	tt.pool.release(root)

	l := tt.merge(origLeft, leftPart)
	r := tt.merge(rightPart, origRight)
...
func (tt *treapTimeline) shallowCopy() *treapTimeline {
	return &treapTimeline{root: tt.root, totalLength: tt.totalLength, prngState: tt.prngState}
}
...
func (tt *treapTimeline) cloneDeepNode(n *treapNode) *treapNode {
	...
	c.value = n.value
	c.size = n.size
	c.priority = xorshift64(&tt.prngState)
...
func (tt *treapTimeline) CloneDeep() *treapTimeline {
	out := &treapTimeline{totalLength: tt.totalLength, prngState: tt.prngState}
	out.root = out.cloneDeepNode(tt.root)

	return out
}

// Erase clears all nodes from the timeline, releasing them to the pool.
func (tt *treapTimeline) Erase() {
	tt.pool.releaseSubtree(tt.root)
	tt.root = nil
	tt.totalLength = 0
}
...
// MergeAdjacentSameValue coalesces consecutive segments with the same time value.
// Reduces node count without changing logical line ownership.
// Uses Segments() + coalesceSegments() + ReconstructFromSegments().
func (tt *treapTimeline) MergeAdjacentSameValue() {
	...
	coalesced := coalesceSegments(segs)
	if len(coalesced) == len(segs) {
		return
	}

	tt.ReconstructFromSegments(coalesced)
}

// coalesceSegments merges adjacent segments with identical Value.
