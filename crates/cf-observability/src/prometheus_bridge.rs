...
//!
//! Builds a Prometheus registry, attaches the OTel→Prometheus exporter as a
//! reader on a `MeterProvider`, and returns the pieces needed to serve the
//! `/metrics` scrape endpoint — a [`PrometheusScrape`] holding the registry,
//! which [`crate::diagnostics`] serves via hyper. Each call creates an
//! independent registry to avoid collector conflicts.
...
mod tests {
    ...
    /// Mirrors the reference suite's `TestPrometheusHandler_ServesMetrics`
    /// (handler builds; the exposition is text/plain). Network serving is
