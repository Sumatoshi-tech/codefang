...
for filepath in ['internal/streaming/benchmark_test.go', 'internal/streaming/integration_test.go']:
    with open(filepath, 'r') as f:
        content = f.read()
    content = re.sub(r'\n\s*"github\.com/Sumatoshi-tech/codefang/internal/analyzers/imports"', '', content)
    with open(filepath, 'w') as f:
        f.write(content)
